...
{
	public class LesserHealingTP : ModProjectile
    {
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Lesser healing Throwable potion");
			aiType = 166;
        }
        ...
        public override void Kill(int timeLeft)
        {
			...
			for(int i = 0; i < 14; i++) {
            int dust = Dust.NewDust(projectile.position, projectile.width, projectile.height, 76, -projectile.velocity.X * 0.5f, -projectile.velocity.Y * 0.5f, 100, new Color(255, 100, 100, 255), 1.65f);
        ...
        public override void AI()
        {
			...
			if (distance < 90f && projectile.position.X < player.position.X + player.width && projectile.position.X + projectile.width > player.position.X && projectile.position.Y < player.position.Y + player.height && projectile.position.Y + projectile.height > player.position.Y) {
				if (player.active) {
					...
					if(!player.HasBuff(BuffID.PotionSickness)){player.statLife += 50;player.HealEffect(50);player.AddBuff(BuffID.PotionSickness,3600);}
				}
			}
			}
				for (int i = 0; i < 200; i++)
